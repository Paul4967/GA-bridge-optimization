...
def mutate_node(bridge_nodes, bridge_connections, base_nodes, all_nodes, base_connections, build_area, grid_size, max_node_offset_multiplier, node_to_mutate):
    ...
    node_offset_multiplier = random.randint(1, round(max_shift_distance))
    ...
    for dx, dy in directions:
        ...
        if new_x > build_area[0] or new_y > build_area[1] or new_x < 0 or new_y < 0:  #is inside domain check
        ...
        # check if movement of connection causes crossing
...
def mutate(mutation_rate, max_node_offset_multiplier, grid_size, build_area, 
    ...
    for node in bridge_nodes:
        if random.random() < mutation_rate:
            random_int = random.randint(1,3) #select 1 of 3 actions
            # delete node
            if random_int == 1:
